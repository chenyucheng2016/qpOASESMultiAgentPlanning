...
PBS fails, the exporter preserves its genuinely conflicted root and static
corridors as \method's input while original CSDO reports search failure.
\method\ may widen only the temporal indexing of the reconstructed static
corridor under the frozen recovery rule. Every source, executable, schedule,
and corridor artifact is hashed. Development uses 24 passing-bay
...
$N=4,8,12,20$, respectively. Exact paired sign tests favor
...
distributed variant. Full continuation reduces median runtime by
\InnerRuntimeReductionPercent\% relative to the inner-only variant and by
\QPContinuationRuntimeReductionPercent\% relative to QP continuation, while
reducing local QP solves by \ContinuationQPReductionPercent\%. All three
variants return valid trajectories. This
...
The values below are pre-provenance development evidence. Protocol v3 must
reproduce them with recorded shared-front-end provenance before submission.
...
cases where the common PBS/root search fails, \method\ repairs
\TurboPBSRecoveries\ (\TurboPBSRecoveryPercent\%), while CSDO repairs none.
Every success is checked by the same continuous validator.

\begin{table}[t]
\caption{Development CSDO evidence (\CSDOCases\ cases; v3 rerun pending).}
\label{tab:csdo}
\centering
\begin{tabular}{lcc}
\toprule
...
The recorded times do not have identical scope and do not support a runtime-win
claim. Median \method\ optimizer-process wall time is
\TurboCSDOMedianWallSeconds~s; median original-CSDO end-to-end process wall
time is \CSDOMedianWallSeconds~s. CSDO is fast precisely because its
