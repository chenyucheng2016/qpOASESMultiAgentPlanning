...
def main():
    ...
    if (configuration.get("protocol_id") != "ral-deterministic-v1"
            or configuration.get("suite") != "manual"
            or configuration.get("track") != "all"
            or tuple(configuration.get("manual_cases", ()))
            != arguments.cases
            or tuple(configuration.get("methods", ()))
            != arguments.expected_methods
            or configuration.get("repetitions") != arguments.repetitions
            or configuration.get("schedule") != "interleaved"
            or configuration.get("schedule_seed") != 20260804
            or configuration.get("timeout_seconds") != 300.0
        ...
        raise ValueError(
            "manifest does not match the frozen deterministic protocol")
    if len(configuration.get("task_order", ())) != len(expected):
        raise ValueError(
            f"deterministic manifest has "
            f"{len(configuration.get('task_order', ()))} tasks; "
    ...
    lines = [
        ...
        f"% {arguments.manifest.name} sha256={sha256(arguments.manifest)}",
