...
int main()
{
    ...
    real_t targets_x[2] = {15.0, 0.0};
    real_t targets_y[2] = {4.8, 5.3};
    real_t tracking_errors[2];
    
    printf("\nTracking errors:\n");
    real_t x0_final = z0[idx_N];
    real_t y0_final = z0[idx_N + 1];
    real_t x1_final = z1[idx_N];
    real_t y1_final = z1[idx_N + 1];
    
    tracking_errors[0] = sqrt(pow(x0_final - targets_x[0], 2) + pow(y0_final - targets_y[0], 2));
    tracking_errors[1] = sqrt(pow(x1_final - targets_x[1], 2) + pow(y1_final - targets_y[1], 2));
    
    printf("  Agent 0: %.3f m\n", tracking_errors[0]);
    printf("  Agent 1: %.3f m\n", tracking_errors[1]);
    
    // Calculate mean and standard deviation
    real_t mean_error = (tracking_errors[0] + tracking_errors[1]) / 2.0;
    real_t variance = (pow(tracking_errors[0] - mean_error, 2) + pow(tracking_errors[1] - mean_error, 2)) / 2.0;
