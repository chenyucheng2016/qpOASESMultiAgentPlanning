/*
 * Test: 8-Agent Crossing Scenario
 *
 * Based on the 6-agent test, this adds two more diagonal agents:
...
{
    printf("================================================================================\n");
    printf("                    8-AGENT TEST WITH CROSSING TRAJECTORIES\n");
    printf("================================================================================\n\n");
    ...
    // Setup neighbors for fully connected 8-agent graph
    int neighbors_0[7] = {1, 2, 3, 4, 5, 6, 7};
    int neighbors_1[7] = {0, 2, 3, 4, 5, 6, 7};
    int neighbors_2[7] = {0, 1, 3, 4, 5, 6, 7};
    int neighbors_3[7] = {0, 1, 2, 4, 5, 6, 7};
    int neighbors_4[7] = {0, 1, 2, 3, 5, 6, 7};
    int neighbors_5[7] = {0, 1, 2, 3, 4, 6, 7};
    int neighbors_6[7] = {0, 1, 2, 3, 4, 5, 7};
    int neighbors_7[7] = {0, 1, 2, 3, 4, 5, 6};
    int* neighbors[NUM_AGENTS] = {neighbors_0, neighbors_1, neighbors_2, neighbors_3, 
                                   neighbors_4, neighbors_5, neighbors_6, neighbors_7};
    int num_neighbors[NUM_AGENTS] = {7, 7, 7, 7, 7, 7, 7, 7};
    ...
    real_t x6_init[4] = {15.0, 15.0, 0.0, 0.0};
    real_t x7_init[4] = {15.0, 0.0, 0.0, 0.0};
    ...
    // Solve
    printf("Solving 8-agent problem...\n");
    ...
    // Print full trajectories (x, y positions for all agents at each timestep)
    printf("Full Trajectories (x, y positions):\n");
    printf("Stage | Agent 0      | Agent 1      | Agent 2      | Agent 3      | Agent 4      | Agent 5      | Agent 6      | Agent 7\n");
    printf("------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|-------------\n");
    for (int k = 0; k <= N; k++) {
        int idx = (k == N) ? (N * (nx + nu)) : (k * (nx + nu));
        printf("  %2d  | (%5.2f,%5.2f) | (%5.2f,%5.2f) | (%5.2f,%5.2f) | (%5.2f,%5.2f) | (%5.2f,%5.2f) | (%5.2f,%5.2f) | (%5.2f,%5.2f) | (%5.2f,%5.2f)\n", k,
               ...
               z_all[6][idx], z_all[6][idx+1],
               z_all[7][idx], z_all[7][idx+1]);
    }

    // Print terminal states
    printf("\nTerminal States (x, y, vx, vy):\n");
    for (int i = 0; i < NUM_AGENTS; ++i) {
        int idx = N * (nx + nu);
    ...
    // Analyze results - check all pairwise distances (28 pairs)
    printf("\nCollision Analysis (all 28 pairs):\n");
    ...
    // Velocity Analysis
    printf("\nVelocity Profile:\n");
    printf("Stage | A0 Speed | A1 Speed | A2 Speed | A3 Speed | A4 Speed | A5 Speed | A6 Speed | A7 Speed |\n");
    printf("------|----------|----------|----------|----------|----------|----------|----------|----------|\n");
    ...
    real_t targets_x[NUM_AGENTS] = {15.0, 0.0, 7.25, 7.75, 15.0, 15.0, 0.0, 0.0};
    real_t targets_y[NUM_AGENTS] = {4.8, 5.3, 15.0, 0.0, 15.0, 0.0, 0.0, 15.0};
