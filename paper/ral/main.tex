...
\IfFileExists{deterministic_results.tex}{\input{deterministic_results.tex}}{}
\title{\LARGE \bf TurboADMM-NL: Nested Parametric Continuation for Nonlinear
...
extends the linear-QP solver TurboADMM to \method, a shared-memory sequential
convex programming method for heterogeneous robot teams. The method
...
On the untouched \PrimaryCases-case split, \method{} validates every case,
stays within \PrimaryMaximumObjectiveGapPercent\% of OSQP, and at $N=20$ is
\WallRatioNTwenty$\times$ faster with \MemoryRatioNTwenty$\times$ lower peak
memory. It repairs \TurboCSDOSuccesses/\CSDOCases{} CSDO-compatible cases,
versus \CSDOSuccesses/\CSDOCases{} for original CSDO.
\fi
\end{abstract}
...
\oldmethod~\cite{Chen2026TurboADMM} addressed a single sequence
...
physically distributed MPC and DDP architectures~\cite{Luis2020DMPC,Saravanos2023DDP}
...
Recent decentralized planners make different coupling choices. MADER and
Robust MADER~\cite{Tordesillas2022MADER,Kondo2023RobustMADER} optimize against
communicated trajectories, with the latter explicitly protecting asynchronous
execution from communication delay. EGO-Swarm~\cite{Zhou2021EGOSwarm} couples
per-agent nonlinear optimization to lightweight topological recovery.
...
collision-free trajectories~\cite{Schulman2013TrajOpt,Mao2016SCvx}. Its performance and
...
Search-then-refine pipelines also smooth conflict-free discrete schedules for
large quadrotor teams~\cite{Honig2018Swarms}.
...
\section{Results}
\label{sec:results}
% Primary results are generated from the frozen, untouched final artifacts.
...
scenario column give agents/obstacles. The execution cap is 300~s; ``--''
denotes no valid completion.}
...
$\DeterministicMinimumObstacleClearance$, accepted under the validator's fixed
$10^{-2}$ numerical feasibility tolerance.
\fi

\ifdefined\PrimaryQpOASESSuccessNFour
\subsection{Untouched Final Evaluation}
The untouched final split tests the predeclared structural crossover.
\else
\subsection{Development Gates}
The frozen development evidence establishes the intended crossover but is not
substituted for the untouched final matrix.
\fi
On \PrimaryCases\ paired scenarios, both
...
$N=4,8,12,20$, respectively. The crossover occurs between $N=8$ and $N=12$ and is
statistically significant at $N=12$ ($p=\WallSignPNtwelve$) and $N=20$
($p=\WallSignPNtwenty$). At $N=20$, centralized OSQP uses
$\MemoryRatioNTwenty\times$ the median peak memory. The median, 95th-percentile,
and maximum absolute objective gaps are \PrimaryMedianObjectiveGapPercent\%,
\PrimaryPninetyfiveObjectiveGapPercent\%, and \PrimaryMaximumObjectiveGapPercent\%,
respectively, below the predeclared 5\% gate.
...
\ifdefined\PrimaryQpOASESSuccessNFour
\begin{table*}[t]
\caption{Untouched final matrix. Entries are valid trajectories/
\PrimaryCasesPerScale{} and median WSL process wall time in seconds among valid
runs. The execution cap is \PrimaryTimeoutSeconds{}~s; ``--'' denotes no valid
completion.}
...
\ifdefined\PrimaryQpOASESSuccessNFour
\subsection{Final Protocol Audit}
The frozen manifest accounts for all $3\times\PrimaryCases$ scheduled
external-method executions as completions or timeout-censored outcomes. All
\method{} and OSQP rows are independently valid and strictly converged, the
objective gate is satisfied, and the generated tables retain every qpOASES
outcome. Solver and benchmark parameters were not changed after the final split
was inspected.
\else
